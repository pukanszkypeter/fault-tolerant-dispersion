...
export class VisualizationComponent implements OnInit {
  ...
  summarize(): void {
    ...
    this.summary = [];
    this.visualizationService.summarize(this.summaryBy.value).subscribe(
      (res) => {
        this.plainSummary = res;
        this.summary = this.createSummary(
          this.plainSummary,
          this.xAxis.value !== 'algorithm'
        );
  ...
  createSummary(steps: any[], afterGraph: boolean): any[] {
    let result = [];
    for (let custom_type_one of afterGraph
      ? this.graphTypes
      : this.algorithmTypes) {
      let series = [];
      let values = steps.filter((step) =>
        afterGraph
          ? step[2] === custom_type_one.valueOf()
          : step[1] === custom_type_one.valueOf()
      );
      for (let custom_type_two of afterGraph
        ? this.algorithmTypes
        : this.graphTypes) {
        let value = values.filter((value) =>
          afterGraph
            ? value[1] === custom_type_two.valueOf()
            : value[2] === custom_type_two.valueOf()
        );
        if (value.length === 1) {
          series.push({
            name: this.languageService.getTranslatedText(
              custom_type_two.valueOf()
            ),
            value: value[0][0],
          });
        } else {
      ...
      result.push({
        name: this.languageService.getTranslatedText(custom_type_one.valueOf()),
        series: series,
      });
    }
    return result;
  ...
  detailByQuery(): void {
    ...
    this.visualizationService
      .detailBy(
        this.detailBy.value,
        this.algorithmTypeDetail.value,
        this.graphTypeDetail.value
      )
      .subscribe(
        (res) => {
          if (res.length > 0) {
