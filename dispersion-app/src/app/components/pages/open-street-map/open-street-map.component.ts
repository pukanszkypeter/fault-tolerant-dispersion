...
export class OpenStreetMapComponent implements OnInit {
  ...
  private defaultLayer: L.TileLayer = L.tileLayer(
  ...
  private cityLayer: L.TileLayer = L.tileLayer(
    'https://{s}.tile.openstreetmap.org/{z}/{x}/{y}.png',
    {
      maxZoom: 18,
      minZoom: 14,
  ...
  removeMarker(coordinate: any): void {
    const index = this.polygonCoordinates.findIndex(
      (latlng) => latlng.lat === coordinate.lat && latlng.lng === coordinate.lng
    );
  ...
  createNetwork(): void {
    this.loading = true;
    this.openStreetMapService.createNetwork(this.polygonCoordinates).subscribe(
      (res) => {
        this.loading = false;
        ...
        for (let i in graph.nodes) {
          ...
          var defaultIcon = L.divIcon({
            iconSize: [25, 25],
            html:
              '<div style="border-radius: 50%; background-color: dodgerblue; justify-content: center; display: flex; align-items: center; color: white; width: 25px; height: 25px;"> ' +
              node.id +
              ' </div>',
          ...
          marker.addEventListener('click', (event) => {
            ...
            const index = this.nodeMarkers.findIndex(
              (marker) =>
        ...
        for (let i in graph.edges) {
          ...
          const fromIndex = this.nodes.findIndex(
          ...
          if (fromIndex > -1 && toIndex > -1) {
            var latlngs = [
              [
                this.nodeMarkers[fromIndex].getLatLng().lat,
                this.nodeMarkers[fromIndex].getLatLng().lng,
              ],
              [
                this.nodeMarkers[toIndex].getLatLng().lat,
                this.nodeMarkers[toIndex].getLatLng().lng,
              ],
            ];
            var polyline = L.polyline(latlngs as [number, number][]).addTo(
              this.map
            );
            ...
            this.randomSimulation = new SimulationStep(
              ...
              new Graph<RandomDispersionNode, RandomDispersionEdge>(
                this.visService.nodes.map(
                  (node) => new RandomDispersionNode(node.id, NodeState.DEFAULT)
                ),
  ...
  getRobots(): number {
    return this.robots.filter((robot) => robot.onID === this.selectedNodeID)
      .length;
  }
    ...
    if (this.algorithmType) {
      ...
      if (robots === 1) {
        var pendingIcon = L.divIcon({
          iconSize: [25, 25],
          html:
            '<div style="border-radius: 50%; background-color: red; justify-content: center; display: flex; align-items: center; color: white; width: 25px; height: 25px;"> ' +
            this.selectedNodeID +
            ' </div>',
          className: 'custom-marker-icon',
        });
  ...
  removeRobot(): void {
    const index = this.robots.findIndex(
      (robot) => robot.onID === this.selectedNodeID
    );
    if (index > -1) {
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      if (nodes[i].state === NodeState.DEFAULT) {
        ...
        icon = L.divIcon({
          iconSize: [25, 25],
          html:
            '<div style="border-radius: 50%; background-color: green; justify-content: center; display: flex; align-items: center; color: white; width: 25px; height: 25px;"> ' +
            nodes[i].id +
            ' </div>',
          className: 'custom-marker-icon',
        });
