...
export function getGraphInputs(type: GraphType): HTMLInputElement[] {
    return Object.keys(graphSettings[type]).map(key => document.getElementById(key) as HTMLInputElement);
}

export function manipulateGraphSettings(type: GraphType, strategy: FormStrategy): void {

    let inputs = getGraphInputs(type);

    if (strategy === FormStrategy.DEFAULT) {
        inputs.forEach(i =>  [i.disabled = true, i.value = graphSettings[type][i.id]]);
    }
    else {
