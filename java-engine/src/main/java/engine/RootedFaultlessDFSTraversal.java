...
public class RootedFaultlessDFSTraversal {
    ...
    public static void main(String[] args) {
        ...
        while (robots.stream().filter(Robot::isSettled).count() != robots.size()) {
            ...
            List<Robot> activeRobots = robots.stream().filter(Predicate.not(Robot::isSettled))
            ...
            // MOVE
            List<Robot> travelers = robots.stream().filter(Predicate.not(Robot::isSettled))
                    .collect(Collectors.toList());
            for (int i = 0; i < travelers.size(); i++) {
                travelers.get(i).setOnID(
                        travelEdge.getFromID().equals(currentNode.getID()) ? travelEdge.getToID()
