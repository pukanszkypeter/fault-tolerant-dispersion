package engine.algorithms.rotor_router_dispersion;
...
@Component
public class RotorRouterDispersionManagement implements
        RotorRouterDispersionManager<RotorRouterDispersionNode, RotorRouterDispersionEdge, RotorRouterDispersionRobot> {

    @Override
    public void step(Graph<RotorRouterDispersionNode, RotorRouterDispersionEdge> graph,
            List<RotorRouterDispersionRobot> robotList) {
        look(graph, robotList);
        compute(graph, robotList);
        move(graph, robotList);
    }

    public void look(Graph<RotorRouterDispersionNode, RotorRouterDispersionEdge> graph,
            List<RotorRouterDispersionRobot> robotList) {
    ...
    public void compute(Graph<RotorRouterDispersionNode, RotorRouterDispersionEdge> graph,
            List<RotorRouterDispersionRobot> robotList) {
    ...
    public void move(Graph<RotorRouterDispersionNode, RotorRouterDispersionEdge> graph,
            List<RotorRouterDispersionRobot> robotList) {
        for (RotorRouterDispersionRobot robot : robotList) {
            if (robot.getState().equals(RobotState.LEADER)) {
                RotorRouterDispersionNode currentNode = graph.getNodeList().stream()
