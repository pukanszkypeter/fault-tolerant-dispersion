...
public class RandomWithLeaderDispersionManagement implements
        RandomWithLeaderDispersionManager<RandomWithLeaderDispersionNode, RandomWithLeaderDispersionEdge, RandomWithLeaderDispersionRobot> {

    @Override
    public void step(Graph<RandomWithLeaderDispersionNode, RandomWithLeaderDispersionEdge> graph,
            List<RandomWithLeaderDispersionRobot> robotList) {
        look(graph, robotList);
        compute(graph, robotList);
        move(graph, robotList);
    }

    private void look(Graph<RandomWithLeaderDispersionNode, RandomWithLeaderDispersionEdge> graph,
            List<RandomWithLeaderDispersionRobot> robotList) {
        /*
         * Map<Long, List<RandomWithLeaderDispersionRobot>> robotsOnDifferentNodes =
         * robotList.stream()
         * .filter(robot -> !robot.getState().equals(RobotState.SETTLED))
         * .collect(groupingBy(RandomWithLeaderDispersionRobot::getOnID));
         * 
         * for (Map.Entry<Long, List<RandomWithLeaderDispersionRobot>> robotsByNode :
         * robotsOnDifferentNodes.entrySet()) {
         * 
         * Map<Color, List<RandomWithLeaderDispersionRobot>> robotsGroupedByColorOnNode
         * = robotsByNode.getValue()
         * .stream().collect(groupingBy(RandomWithLeaderDispersionRobot::getColor));
         * 
         * for (Map.Entry<Color, List<RandomWithLeaderDispersionRobot>> robotsByColor :
         * robotsGroupedByColorOnNode.entrySet()) {
         * // There is at least two leader on this node
         * if (leaderCount(robotsByColor.getValue()) > 1) {
         * List<RandomWithLeaderDispersionRobot> currentLeadersInOneNode =
         * getCurrentLeaders(robotsByColor.getValue());
         * // Recalculate the winner leader
         * RandomWithLeaderDispersionRobot winnerLeader = new
         * LocalLeaderElection<RandomWithLeaderDispersionRobot>().run(
         * currentLeadersInOneNode);
         ...
         * // System.out.println("We have one leader on node:" + robotsByNode.getKey() +
         * " with color: " + robotsByColor.getKey());
         * } else { // => leader count = 0 => need to choose a new one
         * // System.out.println("New leader choosing is happening!");
         * RandomWithLeaderDispersionRobot winnerRobot = new
         * LocalLeaderElection<RandomWithLeaderDispersionRobot>().run(robotsByColor.
         * getValue());
         * robotsByColor.getValue().forEach(robot ->
         * robot.setState(robot.getID().equals(winnerRobot.getID()) ? RobotState.LEADER
         * : RobotState.EXPLORE));
         * }
         * }
         * }
    ...
    private void compute(Graph<RandomWithLeaderDispersionNode, RandomWithLeaderDispersionEdge> graph,
            List<RandomWithLeaderDispersionRobot> robotList) {
        /*
         * Map<Long, List<RandomWithLeaderDispersionRobot>> leadersOnDifferentNodes =
         * robotList.stream()
         * .filter(robot -> robot.getState().equals(RobotState.LEADER))
         * .collect(groupingBy(RandomWithLeaderDispersionRobot::getOnID));
         * 
         * for (Map.Entry<Long, List<RandomWithLeaderDispersionRobot>> leadersByNode :
         * leadersOnDifferentNodes.entrySet()) {
         * 
         ...
         * robot.getOnID().equals(leaderRobot.getOnID())
         * && !robot.getState().equals(RobotState.SETTLED))
         * .forEach(follower -> follower.setState(RobotState.TERMINATED));
         * }
         * 
         * // Need to find a new path
         * List<Long> edgeOptions = getNewRandomPath(graph, leaderRobot.getOnID(),
         * leaderRobot.getColor());
         ...
         * } else {
         * throw new RuntimeException("Leader robot with ID " + leaderRobot.getID() +
         * " stepped into a trap on Node with ID " + leaderRobot.getOnID());
         * }
         * }
         * } else {
         * // Check if the leader is the only robot here and able to settle down
         * if (robotCountOnNode(robotList, leadersByNode.getKey()) == 1) {
         * leadersByNode.getValue().get(0).setDestinationID(leadersByNode.getKey()); //
         * If the destination is the same as the on id it will settle there
         * 
         * } else { // Check if any other group is here IN DIFFERENT COLOR and have
         * election together for settling down
         * 
         * List<RandomWithLeaderDispersionRobot> followersInEveryColor =
         * robotList.stream()
         ...
         * RandomWithLeaderDispersionRobot winnerLeader = new
         * LocalLeaderElection<RandomWithLeaderDispersionRobot>().run(leadersByNode.
         * getValue());
         * 
         ...
         * } else {
         * List<Long> edgeOptions = getNewRandomPath(graph, leader.getOnID(),
         * leader.getColor());
         * 
         * if (edgeOptions.size() > 0) {
         * int random = new RandomNumber().get(0, edgeOptions.size() - 1);
         ...
         * } else {
         * throw new RuntimeException("Leader robot with ID " + leader.getID() +
         * " stepped into a trap on Node with ID " + leader.getOnID());
         * }
         * }
         * }
         * } else {
         ...
         * 
         * new LocalLeaderElection<RandomWithLeaderDispersionRobot>().run(
         * followersInEveryColor).setDestinationID(leadersByNode.getKey());
         * for (RandomWithLeaderDispersionRobot otherLeader : leadersByNode.getValue())
         * {
         * List<Long> edgeOptions = getNewRandomPath(graph, otherLeader.getOnID(),
         ...
         * } else {
         * throw new RuntimeException("Leader robot with ID " + otherLeader.getID() +
         * " stepped into a trap on Node with ID " + otherLeader.getOnID());
         * }
         * }
         * }
         * }
    ...
    private void move(Graph<RandomWithLeaderDispersionNode, RandomWithLeaderDispersionEdge> graph,
            List<RandomWithLeaderDispersionRobot> robotList) {
         ...
         * for (Map.Entry<Color, List<RandomWithLeaderDispersionRobot>> robotsByColor :
         * robotsGroupedByColorOnNode.entrySet()) {
         * RandomWithLeaderDispersionRobot leader =
         ...
         * for (RandomWithLeaderDispersionRobot follower :
         * robotsByColor.getValue().stream().filter(robot ->
     ...
     * }
     * 
     * private Long robotCountOnNode(List<RandomWithLeaderDispersionRobot>
     * robotList, Long nodeID){
     * return robotList.stream().filter(robot ->
     * robot.getOnID().equals(nodeID)).count();
     * }
     * 
     * private List<RandomWithLeaderDispersionRobot>
     * getCurrentLeaders(List<RandomWithLeaderDispersionRobot> robotList){
     * return robotList.stream().filter(robot ->
     * robot.getState().equals(RobotState.LEADER)).collect(Collectors.toList());
     * }
     * 
     * private boolean isNodeOccupied(Graph<RandomWithLeaderDispersionNode,
     * RandomWithLeaderDispersionEdge> graph, Long nodeID){
     ...
     * }
     * 
     * private List<Long> getNewRandomPath(Graph<RandomWithLeaderDispersionNode,
     * RandomWithLeaderDispersionEdge> graph, long onID, Color color){
     ...
     * }
     * 
     * private void settleOnNode(Graph<RandomWithLeaderDispersionNode,
     * RandomWithLeaderDispersionEdge> graph, Long nodeID) {
     * for (RandomWithLeaderDispersionNode node : graph.getNodeList()) {
     ...
     * }
     * 
     * private boolean isComponentOccupied(Graph<RandomWithLeaderDispersionNode,
     * RandomWithLeaderDispersionEdge> graph, Color component) {
     * List<RandomWithLeaderDispersionEdge> componentEdges =
     * graph.getEdgeList().stream().filter(edge ->
     * edge.getColor().equals(component)).collect(Collectors.toList());
     * 
     * List<Long> fromIDs =
     * componentEdges.stream().map(RandomWithLeaderDispersionEdge::getFromID).
     * collect(Collectors.toList());
     * List<Long> toIDs =
     * componentEdges.stream().map(RandomWithLeaderDispersionEdge::getToID).collect(
     * Collectors.toList());
     ...
     * System.out.println(componentNodes.stream().filter(node ->
