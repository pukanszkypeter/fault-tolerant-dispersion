...
def move(simulationState):
    for robot in simulationState.robots:
        ...
        if robot.state == RobotState.SETLER:
            ...
            # set the lastEdgeId for the setled robot
            currentNode = simulationState.getNode(robot.onID)
            if currentNode.state == NodeState.OCCUPIED:
                setledRobot = simulationState.getRobot(list(filter(lambda x: x.state == RobotState.FINISHED and x.onID == robot.onID, simulationState.robots))[0].id)
                setledRobot.lastEdgeID = robot.destinationID
