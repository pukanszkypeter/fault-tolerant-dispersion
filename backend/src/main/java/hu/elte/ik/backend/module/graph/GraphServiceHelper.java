...
public class GraphServiceHelper {
  ...
  public Graph<Node, Edge> generateGraph(
    GraphGenerator<Long, Long, Long> generator
  ) {
  ...
  private static Graph<Node, Edge> convertJGraphToCoreGraph(
    org.jgrapht.Graph<Long, Long> jGraph
  ) {
    ...
    while (vertexIter.hasNext()) {
      ...
      while (edgeIter.hasNext()) {
        ...
        Optional<Edge> _edge = edgeList
          .stream()
          .filter(e -> e.getId().equals(edge))
          .findAny();
        if (_edge.isPresent()) {
