...
@Component
public class RotorRouterHelper
  implements Stepper<RotorRouterNode, RotorRouterEdge, RotorRouterRobot> {

  @Override
  public void step(
    Graph<RotorRouterNode, RotorRouterEdge> graph,
  ...
  public void look(
    ...
    for (RotorRouterRobot robot : robotList) {
      if (
        ...
        robot.getState().equals(RobotState.EXPLORE)
      ) {
        RotorRouterNode currentNode = graph
          .getNodes()
          .stream()
          ...
          .orElseThrow(() ->
            new RuntimeException(
              "Node with ID " + robot.getOnId() + " not found!"
            )
          );
    ...
    for (RotorRouterRobot robot : robotList) {
      if (robot.getState().equals(RobotState.LEADER)) {
        ...
        RotorRouterNode currentNode = graph
          ...
          .orElseThrow(() ->
            new RuntimeException(
              "RotorRouterDispersionNode with ID " +
              robot.getOnId() +
              " not found!"
            )
          );

        List<Long> nodeOptions = graph
          .getEdges()
          .stream()
          .filter(edge ->
  ...
  public void move(
    ...
    for (RotorRouterRobot robot : robotList) {
      if (robot.getState().equals(RobotState.LEADER)) {
        ...
        RotorRouterNode pendingNode = graph
          .getNodes()
          .stream()
          .filter(node -> node.getId().equals(robot.getOnId()))
          .findAny()
  ...
  private void globalLeaderElection(List<RotorRouterRobot> robotList) {
    List<RotorRouterRobot> leaders = robotList
      .stream()
      .filter(robot -> robot.getState().equals(RobotState.LEADER))
      .collect(Collectors.toList());

    for (RotorRouterRobot robot : leaders) {
      List<RotorRouterRobot> nominees = robotList
        .stream()
        .filter(nominee ->
          nominee.getState().equals(RobotState.LEADER) &&
          Objects.equals(nominee.getOnId(), robot.getOnId())
        )
        .collect(Collectors.toList());
      if (nominees.size() > 1) {
        RotorRouterRobot leader = new LocalLeaderElection<RotorRouterRobot>()
          .run(nominees);
