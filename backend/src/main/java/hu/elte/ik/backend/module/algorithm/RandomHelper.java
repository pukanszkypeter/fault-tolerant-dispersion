...
public class RandomHelper
  implements Stepper<RandomNode, RandomEdge, RandomRobot> {

  @Override
  public void step(
    Graph<RandomNode, RandomEdge> graph,
      ...
      if (robot.getState().equals(RobotState.LEADER)) {
        ...
        if (nodeOptions.size() > 0) {
          ...
          throw new RuntimeException(
            "Robot with Id " +
            robot.getId() +
            " stepped into a trap on Node with Id " +
            robot.getOnId()
          );
        }
      }
  ...
  private static void globalLeaderElection(List<RandomRobot> robotList) {
    ...
    for (RandomRobot robot : leaders) {
      List<RandomRobot> nominees = robotList
        ...
        .collect(Collectors.toList());
      if (nominees.size() > 1) {
        RandomRobot leader = new LocalLeaderElection<RandomRobot>()
          .run(nominees);
