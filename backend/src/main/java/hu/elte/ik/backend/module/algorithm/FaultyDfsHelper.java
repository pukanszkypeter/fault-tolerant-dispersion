...
@Component
public class FaultyDfsHelper implements Stepper<FaultyDfsNode, FaultyDfsEdge, FaultyDfsRobot> {

        @Override
        public void step(Graph<FaultyDfsNode, FaultyDfsEdge> graph, List<FaultyDfsRobot> robotList) {
                ...
                List<FaultyDfsEdge> currentEdges = graph
                                .getEdges()
                                .stream()
                                .filter(edge -> currentNode.getId().equals(edge.getFromId())
                ...
                if (!isCurrentNodeOccupied) {
                        ...
                        if (lastRobot.getPhase().equals(RobotPhase.FORWARD) &&
                ...
                FaultyDfsNode travelDestination = graph
                                ...
                                .findAny()
                                .orElse(null);
                if (!inactiveRobots.stream().map(robot -> robot.getOnId()).collect(Collectors.toList())
                                .contains(travelDestination.getId()) && travelers.size() > 0) {
                        travelDestination.setState(NodeState.PENDING);
                }

                for (int i = 0; i < travelers.size(); i++) {
                        travelers.get(i).setOnId(
                                        travelEdge.getFromPort() == exitPort ? travelEdge.getToId()
                                                        : travelEdge.getFromId());
                        travelers.get(i).setLastUsedPort(
                                        travelEdge.getFromPort() == exitPort ? travelEdge.getToPort()
